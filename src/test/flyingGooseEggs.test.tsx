...
describe("goose egg hatching (non-sim)", () => {
  it("creates eggs whose hatch time falls in the 18-32s window", () => {
    // Sample the window at a few deterministic random draws.
    ...
    expect(HATCH_MIN_MS).toBe(18_000);
    expect(HATCH_MAX_MS).toBe(32_000);
  });

  it("does not hatch eggs before their hatchAt timestamp", () => {
    const laidAt = 0;
    const egg = makeEgg({ id: "e1", x: 0, y: 0, laidAt, rand: () => 0 }); // hatchAt = 18s
  ...
  it("hatches every egg once wall-clock time reaches HATCH_MAX_MS", () => {
    const laidAt = 0;
    const eggs = [
      makeEgg({ id: "min", x: 0, y: 0, laidAt, rand: () => 0 }),     // 18s
      makeEgg({ id: "mid", x: 0, y: 0, laidAt, rand: () => 0.5 }),   // 25s
      makeEgg({ id: "max", x: 0, y: 0, laidAt, rand: () => 0.999 }), // ~32s
    ];
    // Just before the minimum: nothing hatches.
    expect(hatchDueEggs(eggs, laidAt + HATCH_MIN_MS - 1).hatched).toHaveLength(0);
    // At the minimum: only the earliest hatches.
    expect(hatchDueEggs(eggs, laidAt + HATCH_MIN_MS).hatched.map((e) => e.id)).toEqual(["min"]);
    // At the maximum: all three hatch deterministically.
